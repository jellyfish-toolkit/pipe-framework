...
from pipe.server.http.load import LBadRequest, LJsonResponse, LNotFound, LResponse
...
class TodoDetailsResource(HTTPPipe):
    pipe_schema = {
        'GET': {
            'out': (
                EDatabase(table_name='todo-items') | LNotFound(), TJsonResponseReady(data_field='todo-items_item'),
                LJsonResponse()
            )
        },
        'POST': {
            'in': (EJsonBody() | LBadRequest(), LDatabase(data_field='json', table_name='todo-items')),
            'out': (TJsonResponseReady(data_field='todo-items_insert'), LJsonResponse(status=201))
        },
        'PUT': {
            'in': (
                ETodoJsonBody() | LBadRequest(),
                TLambda(lambda_=lambda store: store.copy(json=dict(**store.get('json'), **{"id": store.get('id')}))),
                LDatabase(data_field='json', table_name='todo-items')
            ),
            'out': (
                EDatabase(table_name='todo-items'), TJsonResponseReady(data_field='todo-items_item'),
                LJsonResponse(status=201)
            )
        },
        'DELETE': {
            'in': (LDelete(table_name='todo-items'), ),
            'out': (LResponse(status=204), )
        }
    }


if __name__ == '__main__':
    app.run(host='127.0.0.1', port=8080, use_debugger=True, use_reloader=True, use_inspection=True)
